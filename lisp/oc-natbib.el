...
;; "export" capability for citations.  You may activate it globally with
;;
;;    (setq org-cite-export-processor '(natbib))
;;
...
;; will use any "\\usepackage{natbib}" command already present in the documen
...
;;
;; - year(y), including bare(b) variant.
...
(defun org-cite-natbib--build-arguments (citation)
  ...
  (format "{%s}"
          (mapconcat (lambda (ref) (org-element-property :key ref))
